      ...
      <div class="col-md-6 col-lg-7 d-flex justify-content-center align-items-center"
        ...
        <div class="box-container col-12 mb-5">
          ...
          <form @submit.prevent="submitForm" class="align-items-center w-100">
            ...
            <button type="submit" class="btn btn-primary w-100" :disabled="passwordMismatch">Sign Up</button>
          </form>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      email: '',
      password: '',
      confirmPassword: '', // New data property for confirm password
      role: '',
      passwordMismatch: false // Property to track if passwords do not match
    };
  },
  watch: {
    // Watch password and confirmPassword fields for changes and update passwordMismatch accordingly
    password(newVal) {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        ...
        if (result.success) {
          // Store firstname and JWT token in localStorage
          localStorage.setItem('firstname', result.firstname); // Use result.firstname from response
  ...
  mounted() {
    // Redirect authenticated users from accessing the signup page
    const token = localStorage.getItem('jwt_token');
    if (token) {
      this.$router.push('/main'); // Redirect to main page if authenticated
    }
  }
};
</script>
