    ...
    async submitForm() {
      try {
        ...
        if (result.success) {
          ...
          localStorage.setItem('firstname', result.firstname); // Store the first name

          // Redirect based on the route name, assuming result.redirect is "/Home" or "/StudentHome"
          if (result.redirect) {
            this.$router.push({ name: result.redirect.slice(1) }); // Remove the leading slash
          } else {
